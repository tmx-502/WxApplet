...
Page({
  ...
  onLoad: async function (options) {
    ...
    let isDisabled = list[0][1].Evl ? new Date().getTime() - new Date(list[0][1].Evl).getTime() > 2 * 24 * 3600000 : false;
    this.setData({
      options,
  ...
  bhgpz(res) { //其他项上传照片
    ...
    let tempData = 'list[3][' + id[0] + ']';
    if (!this.data.list[3][id[0]].hasOwnProperty('ImgUrl')) this.data.list[3][id[0]].ImgUrl = [];
    wx.chooseImage({
      count: 9 - this.data.list[3][id[0]].ImgUrl.length, //照片上传张数
      sizeType: 'compressed',
      success: (res) => {
        res.tempFilePaths.forEach(item => {
          this.data.list[3][id[0]].ImgUrl.push(item);
  ...
  bindValueChange: function (e) { //检验日期绑定
  ...
  qtListAdd() { //增加其他要判断的项目
    ...
    let index = this.data.list[3].findIndex(e => e.Item.trim() == "");
    if (index == -1) return;
    this.data.list[3][index].isAdd = true;
    this.setData({
      ['list[3][' + index + ']']: this.data.list[3][index]
    })
  },
  input_qt(res) { //其他选项数据回调
    this.setData({
      ['list[3][' + res.currentTarget.id + '].Item']: res.detail.value
    })
  },
  bindPreview(res) { //其他照片预览开关
    ...
    let previewArray;
    if (res.type == "tap") {
      previewArray = this.data.list[3][this.data.bindID.index].hasOwnProperty('ImgUrl') ? this.data.list[3][this.data.bindID.index].ImgUrl : []
    };
    this.setData({
  ...
  addImg(res) {
    ...
    wx.chooseImage({
      ...
      success: (res) => {
        if (!this.data.list[3][this.data.bindID.index].hasOwnProperty('ImgUrl')) {
          this.setData({
            ['list[3][' + this.data.bindID.index + '].ImgUrl']: [],
          })
        }
        res.tempFilePaths.forEach(item => {
          this.data.list[3][this.data.bindID.index].ImgUrl.push(item);
        ...
        this.setData({
          previewArray: this.data.list[3][this.data.bindID.index].ImgUrl
        })
      }
    })
  },
  ...
  deleteImg(res) { //删除照片
    ...
    let _this = this;
    let _res = res;
    let temp = this.data.list[3][this.data.bindID.index].ImgUrl[res.currentTarget.id] //寻找本地图片路径
    let globalImgList = getApp().globalData.imgList;
          ...
          if (res.confirm) {
            getApp().request({
              ...
              if (res.head.status == '1') {
                ...
                getApp().globalData.imgList = globalImgList;
                _this.data.list[3][_this.data.bindID.index].ImgUrl.splice(_res.currentTarget.id, 1);
                _this.setData({
                  previewArray: _this.data.list[3][_this.data.bindID.index].ImgUrl
                })
          ...
          success(res) {
            if (res.confirm) {
              getApp().request({
                ...
                if (res.head.status == '1') {
                  _this.data.list[3][this.data.bindID.index].ImgUrl.splice(res.currentTarget.id, 1);
                  _this.setData({
                    previewArray: _this.data.list[3][_this.data.bindID.index].ImgUrl
                  })
                }
      ...
      async success(res) {
        if (res.confirm) {
          let result = await getApp().request({
            ID: _this.data.list[3].splice(id, 1)[0].ID
          }, 'Write', '20R05');
          _this.setData({
            ['list[' + 3 + ']']: _this.data.list[3]
          })
          wx.showToast({
  ...
  linechange(event) { //记录textarea行数
    this.setData({
      ['textareaheight[' + event.currentTarget.id + ']']: event.detail.height
    })
    console.log(event);
  },
  fromSubmit: async function (e) { //提交数据
    ...
    for (let i = 0; i < 5; i++) {
      ...
      this.data.list[i].forEach(item => {
        ...
        let RecordEvl = i != 3 ? e.detail.value[item.ID] : item.RecordEvl ? '不合格' : '合格';
  ...
  onUnload: function () {
    ...
    getApp().request({
      RecordNo: this.data.options.RecordNo
    }, 'Write', '20R03');
